...
- Service-layer coverage is the strongest part of the suite; `transaction/service.ts`
  and `reward/utility/service.ts` are both above 85% of statements
- Branch coverage is low across the codebase (12.5% overall), which is where the
  suite has the most room to grow
